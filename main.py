...
from utils.Fourier_Estimator_nonSpot import fft_cal_fourier_est
...
def main():
    ...
    x = np.arange(0.03, 0.1, 0.0001) # for simona data
    ...
    obs_processes = np.genfromtxt("p1.csv").reshape(1, N+1)
    timestamp = np.arange(0, N)
    total_obs_prices = np.genfromtxt("p1.csv")
    ...
    if n_traj == 1:
        ...
        FE_whole_est = np.mean(FE_day_estimators, axis=0)  
